...
RSpec.describe TweetsController, type: :controller do
  ...
  context 'POST /tweets' do
    ...
    it 'OK rate limit: 30 tweets per hour' do 
      user = FactoryBot.create(:user)
      session = user.sessions.create 
      @request.cookie_jar.signed['twitter_session_cookie'] = session.token
      ...
      expect(JSON.parse(response.body)['error']['message']).to 
      eq('Rate limit exceeded (30 tweets/hour). Please try again later.')
    end 
  end
